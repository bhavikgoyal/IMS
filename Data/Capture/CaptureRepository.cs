...
namespace IMS.Data.Capture
{
    public class CaptureRepository
    {
        ...
        private List<TreeNode> GetAllNodes()
        {
            ...
            {
                con.Open();
                string query = @"SELECT IndexID, LongIndexName, Parent1Name
                                 FROM Indexes ";
        ...
        private List<TreeNode> BuildTree(List<TreeNode> flatList)
        {
        ...
        private (int fileId, string fileNo, string folderPath) InsertDocumentRow(int indexId, string originalFileName, string fullPath)
        {
            ...
            using (SqlConnection conn = DatabaseHelper.GetConnection())
            {
                ...
                string documentFolder = Path.Combine(tableFolder, fileNo);
                ...
                return (newId, fileNo, documentFolder);
            }
        }
        ...
        public ScanBatch CreateRecordWithoutDocument(string currentUser, out string createdPath)
        {
            ...
            string blankImagePath = Path.Combine(
            ...
            var (fileId, fileNo, folderPath) =
                  InsertDocumentRow(SelectedIndexId, "abcd.jpg", blankImagePath);

            string destPath = Path.Combine(folderPath, "abcd.jpg");
            File.Copy(blankImagePath, destPath, true);

            InsertBlankPageBlob(SelectedIndexId, fileNo, destPath);
        ...
        public void SplitSingleDocument(ScannedDocument doc, List<string> selectedFiles = null)
        {
            ...
            var (newFileId, newFileNo, newDocFolder) =
        InsertDocumentRow(SelectedIndexId, doc.FileNo, currentDocFolder);
            try
            {
     ...
     InsertDocumentRow(SelectedIndexId, doc.FileNo, currentDocFolder);
       ...
       MessageBoxImage.Information);
        ...
        public ScannedDocument ReplicateDocument(ScannedDocument sourceDoc)
        {
            ...
            string sourcePath = sourceDoc.FullPath;
            string fileName = Path.GetFileName(sourcePath);

            var (fileId, fileNo, folderPath) =
        InsertDocumentRow(SelectedIndexId, sourceDoc.FileNo, sourcePath);
            ...
            var newDoc = new ScannedDocument
            {
                FileId = fileId,
                FileNo = fileNo,
                PageNo = 1,
                OriginalFileName = sourceDoc.FileNo,
                FullPath = destFilePath
            };

            batch.Pages.Add(newDoc);
            list.Add(batch);
            ScannedBatches.Add(batch);

            return newDoc;
        }
        public void ApproveSingleDocument(ScannedDocument doc, string currentUser)
        {
            if (doc == null || SelectedIndexId <= 0)
                return;
            ...
            Directory.CreateDirectory(ApproveTablePath);
            ...
            string documentApprovePath = Path.Combine(ApproveTablePath, doc.FileNo);

            try
            {
                if (Directory.Exists(documentApprovePath))
                    Directory.Delete(documentApprovePath, true);
                Directory.Move(documentImportPath, documentApprovePath);
            }
            catch
            {
                return;
            ...
            using (SqlConnection conn = DatabaseHelper.GetConnection())
            {
                ...
                using (SqlCommand cmd = new SqlCommand(sql, conn))
                {
                    ...
                    cmd.Parameters.AddWithValue("@FilePath",
                          string.IsNullOrEmpty(documentApprovePath) ? (object)DBNull.Value : (object)documentApprovePath);
                    cmd.ExecuteNonQuery();
                }
            }
        ...
        public void ApproveAllDocument(string currentUser)
        {
            if (SelectedIndexId <= 0 || ScannedBatches.Count == 0)
                return;
                ...
                try
                {
                    ...
                    else
                    {
                        ...
                        foreach (var dirPath in Directory.GetDirectories(sourceFolder))
                        {
                            ...
                            destSubFolder = Path.Combine(destFolder, folderName);
            ...
            using (SqlConnection conn = DatabaseHelper.GetConnection())
            {
                ...
                using (SqlCommand cmd = new SqlCommand(sql, conn))
                {
                    cmd.Parameters.AddWithValue("@FileNo", folderName);
                    cmd.Parameters.AddWithValue("@User",
                        string.IsNullOrEmpty(currentUser) ? (object)DBNull.Value : currentUser);
                    cmd.Parameters.AddWithValue("@FilePath",
                          string.IsNullOrEmpty(destSubFolder) ? (object)DBNull.Value : (object)destSubFolder);
                    cmd.ExecuteNonQuery();
                }
            }
        ...
        public void DragDropDocumnet(ScannedDocument source, ScannedDocument target, string currentUser)
        {
            if (source == null || target == null || SelectedIndexId <= 0)
                return;
            ...
            string sourceFolder = Path.Combine(tableFolder, source.FileNo);
            string targetFolder = Path.Combine(tableFolder, target.FileNo);
            ...
            var batch = ScannedBatches.FirstOrDefault(b => b.Pages.Any(p => p.FileId == source.FileId));
            if (batch != null)
            {
                ScannedBatches.Remove(batch);
